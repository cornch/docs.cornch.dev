...
<div class="mb-16 px-6 py-4 border dark:border-zinc-500 rounded-sm flex flex-col md:flex-row items-start md:items-center gap-8">
    <dl class="grow flex flex-col gap-y-2">
        ...
        <div class="flex gap-1">
            <dt class="text-zinc-500 shrink-0">{{ __('Updated at') }}</dt>
            <dd class="text-zinc-400">
                <x-datetime :datetime="Date::parse($info['updatedAt'])" :locale="$locale" />
            </dd>
        </div>
        <div class="flex gap-1">
            <dt class="text-zinc-500 shrink-0">{{ __('Translated by') }}</dt>
            <dd>
    ...
    <x-link-button href="{{ $info['crowdinUrl'] }}" target="_blank" rel="noreferrer noopener nofollow" class="shrink-0">
